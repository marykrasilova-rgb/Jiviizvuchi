...
// Copyrighted recordings are not copied to the site. These are short original musical style-clues.
const MODERN_CLUES={
 jazz:[
...
function ensureMqAudio(){
 const AC=window.AudioContext||window.webkitAudioContext;
 if(!AC)return null;
 if(!mqCtx)mqCtx=new AC();
 if(mqCtx.state==='suspended')mqCtx.resume().catch(()=>{});
 return mqCtx;
}
...
function mqTone(ctx,m,t,d,type='sine',gain=.07){
 const o=ctx.createOscillator(),g=ctx.createGain();
 o.type=type;
 o.frequency.value=mqHz(m);
...
function mqChord(ctx,root,t,d,type='triangle',gain=.02){
 [0,7,12].forEach((n,i)=>mqTone(ctx,root+n,t+i*.01,d,type,gain));
}

function playModernClue(){
 if(!mqCurrent)return;
 const ctx=ensureMqAudio();
 if(!ctx){
  const fb=mqEl('modernFeedback');
  if(fb)fb.textContent='На этом устройстве не удалось включить звук.';
  return;
 }
 stopModernClue();
 const meta=mqCurrent[3]||{};
 const tempo=meta.tempo||108;
 const beat=60/tempo;
 const now=ctx.currentTime+.025;
 const feel=meta.feel||'';
 const root=meta.root||48;
 let leadType=mqGenre==='rock'?'sawtooth':mqGenre==='pop'?'triangle':'sine';
 let step=beat*.5;
 if(['ballad','powerballad','strings','felt','tintin','romantic'].includes(feel))step=beat;

 for(let i=0;i<8;i++){
  let t=now+i*beat;
  if(feel==='five')t=now+i*beat*.8;
  if(feel==='waltz')t=now+i*beat*.75;
  mqTone(ctx,root+(i%4===2?7:0),t,beat*.42,mqGenre==='rock'?'square':'sine',.035);
 }

 for(let i=0;i<4;i++){
  const t=now+i*beat*2;
  const rr=root+[0,5,7,0][i];
  if(['disco','popgroove','eighties','electro','synthwave','funk','fusion'].includes(feel)){
   for(let j=0;j<4;j++)mqChord(ctx,rr,t+j*beat*.5,beat*.2,'triangle',.014);
  }else if(['arpeggio','phase','pulse','synthpulse'].includes(feel)){
   for(let j=0;j<8;j++)mqTone(ctx,rr+[0,7,12,7][j%4],t+j*beat*.25,beat*.18,'sine',.02);
  }else{
   mqChord(ctx,rr,t,beat*1.2,mqGenre==='rock'?'sawtooth':'triangle',.016);
  }
 }
 ...
 mqCurrent[2].forEach((m,i)=>{
  ...
  mqTone(ctx,m,t,step*.76,leadType,mqGenre==='academic'?0.065:0.075);
 });
}

function buildModernQuestion(){
 const works=modernQuizWorks();
 if(!works.length)return false;
 if(!mqQueue.length)mqQueue=mqShuffle(works);
 mqCurrent=mqQueue.shift();
 const round=mqEl('modernRound');
 const score=mqEl('modernScore');
 const title=mqEl('modernQuizTitle');
 const feedback=mqEl('modernFeedback');
 const answers=mqEl('modernAnswers');
 if(!round||!score||!title||!feedback||!answers)return false;
 round.textContent=`${mqRound+1}/10`;
 score.textContent=mqScore;
 title.textContent=(typeof genreData!=='undefined'&&genreData[mqGenre])?genreData[mqGenre].title:'Угадай';
 feedback.textContent='';
 const wrong=mqShuffle(works.filter(w=>w[0]!==mqCurrent[0])).slice(0,3).map(w=>w[0]);
 const names=mqShuffle([mqCurrent[0],...wrong]);
 answers.innerHTML=names.map(n=>`<button type="button" data-modern-answer="${n.replaceAll('&','&amp;').replaceAll('"','&quot;')}">${n}</button>`).join('');
 answers.querySelectorAll('button').forEach(btn=>{
  btn.addEventListener('click',()=>answerModernQuestion(btn,btn.dataset.modernAnswer));
 });
 return true;
}
...
function answerModernQuestion(btn,name){
 ...
 if(name!==mqCurrent[0]){
  ...
  if(typeof recordGameAnswer==='function')recordGameAnswer('modern',false);
  return;
 }
 btn.classList.add('correct');
 mqEl('modernAnswers')?.querySelectorAll('button').forEach(b=>b.disabled=true);
 mqScore++;
 if(mqEl('modernScore'))mqEl('modernScore').textContent=mqScore;
 ...
 mqRound++;
 if(mqRound>=10){
  setTimeout(showModernFinish,320);
  return;
 }
 setTimeout(()=>{
  if(buildModernQuestion())playModernClue();
 },320);
}

function showModernFinish(){
 stopModernClue();
 mqEl('modernQuizPlay')?.classList.add('hidden');
 const finish=mqEl('modernFinish');
 if(!finish)return;
 ...
 document.getElementById('modernAgain')?.addEventListener('click',()=>startModernQuiz(mqGenre));
}
...
function startModernQuiz(key){
 ...
 mqRound=0;
 mqScore=0;
 mqQueue=mqShuffle(modernQuizWorks());
 document.getElementById('modernHub')?.classList.add('hidden');
 mqEl('modernFinish')?.classList.add('hidden');
 mqEl('modernQuizPlay')?.classList.remove('hidden');
 ensureMqAudio();
 if(buildModernQuestion())playModernClue();
 window.scrollTo({top:0,behavior:'smooth'});
}

function closeModernQuiz(){
 stopModernClue();
 mqEl('modernQuizPlay')?.classList.add('hidden');
 mqEl('modernFinish')?.classList.add('hidden');
 document.getElementById('modernHub')?.classList.remove('hidden');
 window.scrollTo({top:0,behavior:'smooth'});
}

function initModernQuiz(){
 mqEl('modernListen')?.addEventListener('click',playModernClue);
 mqEl('modernQuizBack')?.addEventListener('click',closeModernQuiz);
 document.querySelectorAll('#modernHub [data-genre]').forEach(btn=>{
  btn.addEventListener('click',()=>startModernQuiz(btn.dataset.genre));
 });
}

if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initModernQuiz);
else initModernQuiz();
